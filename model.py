...
class Attention(nn.Module):
    ...
    def forward(self, encoder_out, decoder_hidden):
        ...
        att2 = self.decoder_att(decoder_hidden)                      # (batch_size, 1, attention_dim)
...
class DecoderWithAttention(nn.Module):
    ...
    RNN taking the encoded images as input to predict corresponding sentences with attention on encoding output
    """
    def __init__(self, attention_dim, embedding_dim, decoder_dim, vocab_size, encoder_dim=2048, dropout=0.3):
        ...
        # Here we use a LSTMCell (step-by-step) instead of LSTM (one step by sequence).
        # (For convenience only in this case)
        ...
        self.classifier = nn.Linear(decoder_dim, vocab_size)  # linear layer to find scores over vocabulary
    ...
    def forward(self, encoder_out, encoded_captions, caption_lengths):
        ...
        # Embedding
        embeddings = self.embedding(encoded_captions)  # (batch_size, max_caption_length, embedding_dim)
        ...
        for t in range(max(decode_lengths)):
            ...
            preds = self.classifier(self.dropout(h))  # (batch_size_t, vocab_size)
...
class Decoder(nn.Module):
    ...
    def forward(self, encoder_out, encoded_captions, caption_lengths):
        ...
        Return:
        -------
        scores for vocabulary, sorted encoded captions, decode lengths, sort indices
        """
        batch_size = encoder_out.size(0)
        encoder_dim = encoder_out.size(-1)
        vocab_size = self.vocab_size
